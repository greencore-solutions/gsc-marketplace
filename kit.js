...
`); });
  ...
  app.get("/.well-known/agent-card.json", (req, res) => { const h = H(req); json(res, {
    ...
    skills: [
      ...
      { id: "x402-capability", name: "x402 capability", description: `x-gsc-x402: ready · ${POSTURE.x402.network} ${POSTURE.x402.chain} ${POSTURE.x402.asset} · ${POSTURE.x402.status} · single paid endpoint https://${HOSTS.apex}/x402/resolve, terms in its 402 payload only.`, tags: ["x402","capability"] },
