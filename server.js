...
// in every complete answer. Open endpoints; the single paid endpoint is /api (x402.js).
...
const envelope = (body, extra = {}) => ({
  ...
  this_server: { mcp: MCP_URL, tools: TOOL_NAMES, registry: REGISTRY, posture: "open — no authentication, no payment on this MCP; x402 live: the single paid endpoint is https://gsc-marketplace.ai/api, terms in its 402 payload only (x-gsc-x402: ready)" },
...
function buildMcp() {
  ...
  mcp.registerTool("get_x402_rail", {
    ...
    description: "What GSC-Marketplace declares about x402: header, network, asset, status — and where the single paid endpoint lives (https://gsc-marketplace.ai/api; its terms are stated in its 402 payload only, never here). Door: https://gsc-marketplace.ai/x402 · skill: x402-settlement.",
    inputSchema: {},
  }, async () => t(envelope({ signal: "ACM-200", state: "ALLOW", posture: POSTURE.v1, x402: POSTURE.x402, door: "https://gsc-marketplace.ai/x402", paid_endpoint: "https://gsc-marketplace.ai/api", skill: "https://gsc-marketplace.ai/.well-known/agent-skills/x402-settlement/SKILL.md", wire: "every gen-2 GSC response carries x-gsc-x402: ready (header 19 of the Ghost Nineteen)" })));
...
// x402 — the live test endpoint (open door; /api is the single paid endpoint)
